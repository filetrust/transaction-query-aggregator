...
{
    [TestFixture]
    public class WhenAllEndpointsReturnOk : TransactionServiceTestBase
    {
        ...
        public async Task Setup()
        {
            ...
            HttpClient.Setup(s =>
                    s.SendAsync<GetTransactionsResponseV1>(It.Is<GlasswallHttpRequest>(x => x.FullPath == $"{Endpoint1}/api/v1/transactions"), It.IsAny<CancellationToken>()))
        ...
        public void Expected_Output_Is_Returned()
        {
            ...
            Assert.That(_output.Files, Has.Exactly(5).Items);
            Assert.That(_output.Count, Is.EqualTo(5));
        }
    }
}